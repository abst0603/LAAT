	       ...
	       Options const &options)
{
  ...
  for (DataPoint *ant : ants)
  {
    DataPoint *nextPoint = ant;
    accumulatedPheromone.assign(data.size(), 0);
    vector<float> V;
    vector<float> P;
	...
	for (size_t dim = 0; dim < 3; ++dim)
	  relativeDistances[j][dim] = neighbor[dim] - currentPoint[dim];
	
	// normalize distance
	float norm = sqrt(
	  relativeDistances[j][0] * relativeDistances[j][0] +
	  relativeDistances[j][1] * relativeDistances[j][1] +
	  relativeDistances[j][2] * relativeDistances[j][2]);

	for (float &dim : relativeDistances[j])
	  dim /= norm;
      }
	...
	cout << "Warning: no suitable neighbors were found in ant search\n";
	...
	data[idx].addPheromone(accumulatedPheromone[idx]);
  }
}
